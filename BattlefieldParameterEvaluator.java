...
public class BattlefieldParameterEvaluator {
	...
	final static int NUMSAMPLES = 1000; // Number of inputs for the multilayer
										// perceptron (size of the input
										// vectors)
	final static int NUM_NN_INPUTS = 2; // Number of hidden neurons of the
										// neural network
	final static int NUM_NN_HIDDEN_UNITS = 20; // Number of epochs for training
	final static int NUM_TRAINING_EPOCHS = 100000;

	final static int MIN_TRAINING_EPOCHS = 100;
	...
	public static void main(String[] args) {
		...
		// Run from C:/Robocode
		RobocodeEngine engine = new RobocodeEngine(new java.io.File("D:/robocode"));
		...
		for (NdxSample = 0; NdxSample < NUMSAMPLES * 2 / 3; NdxSample++) {
			// IMPORTANT:normalize the inputs and the outputs to the interval
			// [0,1]
		...
		do {
			...
			if (err > previousErr) {
				...
				if(err< bestErr){
					...
					System.out.println("Epoch #" + epoch + " Best network: " + bestErr);
				}
			}
			previousErr = err;
		...
		for (int NdxBattleSize = 0; NdxBattleSize < NUMBATTLEFIELDSIZES; NdxBattleSize++) {
			for (int NdxCooling = 0; NdxCooling < NUMCOOLINGRATES; NdxCooling++) {
				...
				topNN.compute(MyTestData[NdxCooling + NdxBattleSize * NUMCOOLINGRATES], output);
